...
# 4. 同步构建产物与部署脚本
echo "[5/5] 同步构建产物到服务器..."
rsync -az --delete -e "ssh $RSYNC_SSH_ARGS" dist/ "${REMOTE_TARGET}:${SERVER_REPO}/dist/"
rsync -az --delete -e "ssh $RSYNC_SSH_ARGS" server/dist/ "${REMOTE_TARGET}:${SERVER_REPO}/server/dist/"
rsync -az -e "ssh $RSYNC_SSH_ARGS" deploy/ "${REMOTE_TARGET}:${SERVER_REPO}/deploy/"

echo "  触发远端部署..."
ssh "${SSH_ARGS[@]}" "${REMOTE_TARGET}" "SKIP_GIT_SYNC=1 bash ${SERVER_REPO}/deploy/deploy-server.sh"
echo ""
echo "============================================"
echo "  构建完成"
echo "============================================"
